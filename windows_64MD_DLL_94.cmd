...
if "%NEW_WRAP%"=="with_new_wrap" (
    ...
    node -e "const fs = require('fs'); fs.writeFileSync('buildtools/third_party/libc++/BUILD.gn', fs.readFileSync('buildtools/third_party/libc++/BUILD.gn', 'utf-8').replace(/(\/\/third_party\/libc\+\+\/src\/src\/verbose_abort\.cpp\",)/, '$1\n    \"//third_party/libc++/src/src/wrap_symbols.cc\",'));
